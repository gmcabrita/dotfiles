...
When using the Elixir programming language:

- For finding direct callers use `mix xref callers Module.function/arity`
- For dependency / compile graph analysis use `mix xref graph`
- For finding source definitions use `rg "def(module|macro)?\s+Name"` or `sg` where applicable
- For browsing documentation use `h Module.function/arity` in `iex -S mix`, `mix help TASK`, or `mix hex.docs online PACKAGE`
