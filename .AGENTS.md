...
When using the Rust programming language:

- For finding APIs or callers use `rust-analyzer`, if it's not available install it using `rustup component add rust-analyzer rust-src && mise reshim`
- For browsing crate documentation use `rustdoc-text --online CRATE ITEM_PATH` and read the text
- For browsing the standard library use `rustup doc --path TOPIC` and read the html,
  - e.g. `rustup doc --path std::collections::BTreeMap`
