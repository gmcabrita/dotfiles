with
    data as (
        select
            ...
            round(100.0 * p.dead_tuple_bytes / p.max_dead_tuple_bytes, 2) as dead_pct,
            p.dead_tuple_bytes,
...
select
    ...
    || case
        ...
        then round(dead_tuple_bytes::numeric / 10 ^ 9::numeric, 0)::text || 'B '
        when dead_tuple_bytes > 10 ^ 6
        then round(dead_tuple_bytes::numeric / 10 ^ 6::numeric, 0)::text || 'M '
        when dead_tuple_bytes > 10 ^ 3
        then round(dead_tuple_bytes::numeric / 10 ^ 3::numeric, 0)::text || 'k '
    ...
    || case
        ...
        then round(max_dead_tuple_bytes::numeric / 10 ^ 9::numeric, 0)::text || 'B '
        when max_dead_tuple_bytes > 10 ^ 6
        then round(max_dead_tuple_bytes::numeric / 10 ^ 6::numeric, 0)::text || 'M '
        when max_dead_tuple_bytes > 10 ^ 3
        then round(max_dead_tuple_bytes::numeric / 10 ^ 3::numeric, 0)::text || 'k '
