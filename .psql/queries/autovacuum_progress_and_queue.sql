    ...
    vacuum_settings as (
        select
            ...
            case
                ...
                then
                    regexp_replace(
                        relopts, '.*autovacuum_vacuum_threshold=([0-9.]+).*', '\1'
                    )::int8
            ...
            case
                ...
                then
                    regexp_replace(
                        relopts, '.*autovacuum_vacuum_scale_factor=([0-9.]+).*', '\1'
                    )::numeric
...
select
    ...
    round(
        (100 * psat.n_dead_tup::numeric / nullif(pg_class.reltuples, 0))::numeric, 2
    ) as dead_tup_pct,
