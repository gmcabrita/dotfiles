...
brew install --formula git gh sqlite duckdb fzf dos2unix colordiff bash libyaml jemalloc mise git-extras git-delta gnu-time jq fq less moreutils ncdu ripgrep rga poppler grep rlwrap scc asciinema tree libpq cloud-sql-proxy glow gum mods vhs google-cloud-sdk gnu-tar gpg htop tailspin 1password-cli yt-dlp pgcli litecli hyperfine k9s rbspy vegeta ugrep btop neofetch fswatch elixir-ls just typst zola sqlc terraform sad httpstat gitleaks semgrep gptscript gptscript-ai/tap/clio jj pg_top jd spacer zoxide rclone duf eza fd doggo
# brew install --formula rustup
